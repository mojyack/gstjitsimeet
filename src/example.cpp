...
auto run() -> bool {
    ...
    assert_b(gst_element_set_state(pipeline.get(), GST_STATE_PLAYING) == GstStateChangeReturn::GST_STATE_CHANGE_SUCCESS);

    const auto bus = AutoGstObject(gst_element_get_bus(pipeline.get()));
